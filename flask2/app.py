    ...
    if request.method == "POST" and form.validate_on_submit():
        ...
        if file and file.filename:
            ...
            if is_forbidden_file(original_filename):
                flash(
                    f"Загрузка файлов типа {get_file_extension(original_filename)} запрещена!",
                    "error",
                )
            ...
            if not is_allowed_file(original_filename):
                flash(
                    f"Тип файла {get_file_extension(original_filename)} не поддерживается. Разрешены: {', '.join(UploadForm.allowed_extensions)}",
                    "error",
                )
                return redirect(url_for("index"))
            ...
            temp_path = os.path.join(
                app.config["UPLOAD_FOLDER"], "_temp_" + original_filename
            )
